...
<body>
                ...
                <div class="form-grid">
                    ...
                    @if(Auth::user()->isSuperAdmin())
                        ...
                        <div class="form-group full-width" id="operator_selection" style="display: none;">
                            <label class="form-label">Opérateur(s) *</label>
                            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
                                <span id="operator_counter" style="font-size:12px;color:var(--muted);">0 opérateur(s) sélectionné(s)</span>
                                <div style="display:flex;gap:6px;">
                                    <button type="button" onclick="selectAllOperators()" class="btn btn-secondary" style="font-size:11px;padding:3px 10px;">Tout sélectionner</button>
                                    <button type="button" onclick="deselectAllOperators()" class="btn btn-secondary" style="font-size:11px;padding:3px 10px;">Tout désélectionner</button>
                                </div>
                            </div>
                            <div id="operator_checkboxes"
                        ...
                        <div class="form-group full-width" id="substore_selection" style="display: none;">
                            ...
                            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
                                <span id="substore_counter" style="font-size:12px;color:var(--muted);">0 sub-store(s) sélectionné(s)</span>
                                <div style="display:flex;gap:6px;">
                                    <button type="button" onclick="selectAllSubstores()" class="btn btn-secondary" style="font-size:11px;padding:3px 10px;">Tout sélectionner</button>
                                    <button type="button" onclick="deselectAllSubstores()" class="btn btn-secondary" style="font-size:11px;padding:3px 10px;">Tout désélectionner</button>
                                </div>
                            </div>
                            <div id="substore_checkboxes"
                                style="max-height:280px;overflow-y:auto;border:1px solid var(--input-border);border-radius:8px;background:var(--input-bg);padding:4px;">
    ...
    <script>
        ...
        function checkCampaignVisibility() {
            ...
            // For sub-store type: show campaigns if at least one sub-store is selected
        ...
        function updateOperatorCounter() {
            ...
            counter.textContent = selected === 0 ? 'Aucun opérateur sélectionné' : `${selected} opérateur(s) sélectionné(s)`;
        }
        ...
        function updateSubstoreCounter() {
            ...
            counter.textContent = selected === 0 ? 'Aucun sub-store sélectionné' : `${selected} sub-store(s) sélectionné(s)`;
        }
        ...
        async function loadCampaignsForSelectedSubstores() {
            ...
            try {
                ...
                if (allCampaigns.length === 0) {
                    campaignCheckboxes.innerHTML = '<div style="text-align:center;padding:20px;color:var(--muted);">Aucune campagne trouvée.</div>';
                } else {
